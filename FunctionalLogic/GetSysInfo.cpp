...
*/
 ...
 bool isExistJournalctl=false;
 bool isExistSystlog=false;
 bool isExistDmesg=false;
...
int n_count;
    ...
    if (exist("/usr/bin/journalctl")||(  exist("/bin/journalctl")   ))
    {

        isExistJournalctl=true;
   ...
   //cout<<"cmd.c_str()="<<cmd.c_str()<<endl; getchar();
    ...
    if( !isExistJournalctl )//判断  `/var/log/sys`
    {

      stream = popen("ls /var/log/syslog*", "r");   
  
string  rt_str=GetStdoutFromCommand("ls /var/log/syslog*",n_count);  //C++封装后的popen
 
 if(n_count!=0)
 {
     std::vector<string>  rt_splitstr= splitGetName(rt_str,"\n");

// for(int i=0;i<rt_splitstr.size();i++)
// {
//   cout<<"i="<<rt_splitstr[i]<<endl;
// }
if(IsFolderExist("/var/log/usblog")==0)
{
    system("mkdir /var/log/usblog > /dev/null 2>&1 &");

}
else 
{
  system("rm /var/log/usblog/* > /dev/null 2>&1 &");

}

for(int i=0;i<rt_splitstr.size();i++)  //解压syslog.gz*到 usblog/下
{
 std::size_t  nPos=rt_splitstr[i].find(".gz");
 // std::size_t  nPosHeard=rt_splitstr[i].find("/var/log/");
if( nPos!=string::npos)
{
 std::size_t  nPosHear=rt_splitstr[i].find("syslog");

 
string gunzip_str="gzip"+rt_splitstr[i];
string t_name=rt_splitstr[i].substr(nPosHear,nPos-nPosHear);
string unzipName="/var/log/usblog/";
unzipName+=t_name;
gunzip_str+=" > ";

 gunzip_str+=unzipName;
gunzip_str+=" > /dev/null 2>&1 &";

cout<<"gunzip_str="<<gunzip_str<<endl;
  system(gunzip_str.c_str());
  
}
    else
    {
      string cm_Copy="cp ";
      cm_Copy+=rt_splitstr[i];
      cm_Copy+=" /var/log/usblog/";
      cm_Copy+="  > /dev/null 2>&1 &";
      cout<<"cm_Copy= "<<cm_Copy<<endl;
      system(cm_Copy.c_str());
    }
 ...
 else  
 {
  ...
  {
  //  if (stream) {
  while (!feof(stream)|| n_count!=0)
  {

    int n_countIner;
 string  rt_str=GetStdoutFromCommand("ls /var/log/usblog/syslog*",n_countIner);  //C++封装后的popen
 std::vector<string>  rt_splitstr= splitGetName(rt_str,"\n");
    vector< std::string> rt_txt;
    vector< std::string> final_path_txt_name;
 
 for(int i=0;i<rt_splitstr.size();i++)
 {
    string path = rt_splitstr[i];
    std::string str_temp;
    
 cout<<"path="<<path<<endl;
   // rt_txt = readtxt_name(path);//take_bug?没写完？？
...
cout<<"in .so :GetSysInfo::GetSysInfo()"
<<endl;
