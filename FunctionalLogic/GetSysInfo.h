...
GetSysInfo();
std::vector<st_sysAccount> getSysAccountSecurityCheck();//[系统用户信息]1.系统账号安全检查

  st_sysinfo getOperationSystemInfo();  //[操作系统信息]12:（国产机设备）硬件设备

 void getFileSystemInfo(vector<st_Hardware_lshw> &get_disk);//    lshw -c disk

std::vector<st_fileSystem>getFileSystemInfo_InC();//3.文件系统和文件挂载检查  暂时不用

 std::vector<st_SystemModuleSecurityCheck> getSystemModuleSecurityCheck();// [2]系统模块检查
