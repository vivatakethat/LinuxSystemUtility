...
namespace SimpleWeb {
  ...
  protected:
   ...
   // std::cout<<__LINE__<<__FUNCTION__<<"host="<<host<<",port="<<port<<std::endl;
    // asio::ip::tcp::resolver::query query("192.168.2.129", "8301");
     //192.168.2.129:33446/
      ...
      resolver->async_resolve(query, [this, connection, resolver](const error_code &ec, asio::ip::tcp::resolver::iterator it) {
        ...
        if(!lock)
          return;
       // std::cout<<__LINE__<<__FUNCTION__<<"ec="<<ec.message()<<std::endl;//take_debug:
        if(!ec) {
          connection->set_timeout(this->config.timeout_request);
          asio::async_connect(*connection->socket, it, [this, connection, resolver](const error_code &ec, asio::ip::tcp::resolver::iterator /*it*/) {
            connection->cancel_timeout();//take_debug:
            auto lock = connection->handler_runner->continue_lock();
            if(!lock)
              return;
             std::cout<<"Connect "<<host<<":"<<port<<"  Fail! "<<"ec.message()="<<ec.message()<<",line:"<<__LINE__<<"function:"<<__FUNCTION__<<std::endl;//take_debug:服务端 To clear up the firewall rules for an external firewall, you can use this command: iptables -F
            if(!ec) {
